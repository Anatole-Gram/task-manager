...
export const useTasks = defineStore('tasks', {
    ...
    actions: {
        async getTasks(id) {
            const response = await fetch(`${new URL(this.url)}tasks?id=${id}&last=${this.lastUpd.updatedAt}`, { method: "GET" });
        ...
        async removeTask(id) {
            const response = await fetch(new URL(`${this.url}tasks/rm-task?id=${id}`), {method: "DELETE"})
            if (response.ok) {
        ...
        async rmTodo(id, taskId, destination) {
            const response = await fetch(new URL(`http://project-lucy.space/api/todos/rm-todo?id=${id}&task=${taskId}&destination=${destination}`), {method: "DELETE"})
            if (response.ok) {
